...
public final class TemplatePluginRequest implements PluginRequestInternal {
    ...
    public static TemplatePluginRequest parsePluginRequest(String pluginCoords) {
        String[] parts = pluginCoords.split(":");
        if (parts.length != 2 || Strings.isNullOrEmpty(parts[0]) || Strings.isNullOrEmpty(parts[1])) {
            throw new IllegalArgumentException(String.format("Invalid plugin spec: '%s' found in System Property '%s'. Expected format is 'id:version'.", pluginCoords, TemplatePluginHandler.TEMPLATE_PLUGINS_PROP));
        }
        return new TemplatePluginRequest(parts[PLUGIN_ID_INDEX], parts[PLUGIN_VERSION_INDEX]);
    }

    public TemplatePluginRequest(String pluginId, String pluginVersion) {
